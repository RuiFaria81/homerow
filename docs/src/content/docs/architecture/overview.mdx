...
![Homerow architecture layers from infrastructure to deploy orchestration](/images/architecture-overview.svg)
...
```mermaid
flowchart LR
  U["User Browser"] --> NG["nginx + TLS (ACME)"]
  NG --> WM["Custom Webmail Service"]
  WM --> PG["PostgreSQL (indexed mailbox data)"]
  SE["Sync Engine"] --> PG
  SE <--> IMAP["Postfix + Dovecot (IMAP/SMTP)"]
  WM --> IMAP
  NG --> RSPAMD["Rspamd"]
  VPS["VPS (NixOS)"] -. "restic backups" .-> S3["Object Storage (encrypted backups)"]
```
