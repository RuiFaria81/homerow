...
expect_contains 'TF_STATE_STACK=${TF_STATE_STACK:-"hetzner-object-storage"}'
expect_contains 'TF_STATE_STACK_DIR="infra/terraform-state/${TF_STATE_STACK}"'
...
expect_contains 'destroy_tf_state_stack() {'
expect_contains 'if ! terraform -chdir="${dir}" import -input=false \'
expect_contains '-var="location=${HETZNER_OBJECT_STORAGE_LOCATION}" \'
expect_contains '-var="s3_access_key=${S3_ACCESS_KEY:-}" \'
expect_contains '-var="s3_secret_key=${S3_SECRET_KEY:-}" \'
...
expect_contains 'destroy_stack "${DNS_STACK_DIR}" "[3/4] Destroying DNS stack..." "${TEMP_TF_BACKEND_DNS}"'
...
expect_contains 'destroy_stack "${VPS_STACK_DIR}" "[4/4] Destroying VPS stack..." "${TEMP_TF_BACKEND_VPS}"'
